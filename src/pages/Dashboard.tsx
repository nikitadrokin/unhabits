...
export function Dashboard() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold mb-8">Your Unhabits</h1>
      
      {activeUnhabits.length === 0 ? (
        <div className="text-center py-12">
          <h3 className="text-lg font-medium text-gray-900 mb-2">No unhabits yet</h3>
          <p className="text-gray-500 mb-4">Start by creating your first unhabit to track</p>
          <Link
            to="/new"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {activeUnhabits.map((unhabit) => {
            const unhabitLogs = logs.filter((log) => log.unhabitId === unhabit.id);
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-xl font-semibold text-gray-900">{unhabit.name}</h3>
                  <button
